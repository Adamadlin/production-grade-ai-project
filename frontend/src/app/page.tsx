...
//         }
//       } catch {}
//       setToast({ show: true, type: 'success', message: 'Ingest complete.' });
//     } catch (e:any) {
...
export default function Home() {
  ...
  // Load health + examples; restore settings
  useEffect(() => {
    (async () => {
      try {
        const h = await api.health();
      ...
      try {
        const ex = await api.examples(6, collection);
        setExamples(ex.examples?.length ? ex.examples : ['emperor','roman army tactics','history of microsoft']);
      } catch {
        setExamples(['emperor','roman army tactics','history of microsoft']);
      }
      const saved = localStorage.getItem('pgai.settings');
      if (saved) {
        try {
  ...
  useEffect(() => {
    localStorage.setItem('pgai.settings', JSON.stringify({ model, k, temperature, maxTokens, collection }));
  }, [model, k, temperature, maxTokens, collection]);
  ...
  async function onSend(e?: React.FormEvent) {
    ...
    try {
      if (mode === 'summarize') {
        const res = await api.summarize(text, { k, model, temperature, max_tokens: maxTokens, collection });
        setMessages(m => [...m, { id: crypto.randomUUID(), role: 'assistant', content: `**Summary** *(model: ${res.model}, collection: ${res.collection})*\n\n${res.summary}` }]);
      } else {
        const res = await api.search(text, 5, { collection });
        const md = [
          `**Search Results for:** ${res.query} *(collection: ${res.collection})*`,
          ...res.results.map((r: any, i: number) => {
            const url = r.citation?.match(/\((.+?):\d+-\d+\)/)?.[1] ?? r?.meta?.source ?? '#';
            return `**${i+1}.** ${r.text}\n\n> [${r.citation}](${url})`;
          })
  ...
  async function runIngest() {
    ...
    try {
      const list = urls.split('\n').map(s=>s.trim()).filter(Boolean);
      const res = await api.ingest(list, tokens, overlap, force, collection);
      setIngestOut(res);
    } catch (e:any) {
      setIngestOut({ error: e.message });
    } finally {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
          ...
          <div className="hidden md:flex items-center gap-3">
            <div className="flex items-center bg-slate-900 border border-slate-700 rounded-lg overflow-hidden">
              <button
                ...
                onClick={()=>setMode('search')}
                disabled={busy}
                title="Semantic search across your indexed data"
              >Search</button>
              <button
                className={`px-3 py-1 text-sm ${mode==='summarize'?'bg-slate-800':''}`}
                onClick={()=>setMode('summarize')}
                disabled={busy}
                title="Generate a concise, cited summary"
              >Summarize</button>
            </div>
            ...
            {mode==='summarize' && (
              <>
                <select className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-1" value={model} onChange={e=>setModel(e.target.value)} title="Model">
                  <option>llama3:8b</option>
                  <option>qwen2.5:3b-instruct</option>
                </select>
                <label className="text-xs" title="Top-k retrieved chunks (higher = more recall, slower)">
                  k <input className="w-14 ml-1 bg-slate-900 border border-slate-700 rounded px-2 py-1" type="number" value={k} onChange={e=>setK(Number(e.target.value))}/>
                </label>
                <label className="text-xs" title="Creativity (0 = factual/precise, 1 = creative)">
                  temp <input className="w-20 ml-1 bg-slate-900 border border-slate-700 rounded px-2 py-1" type="number" step={0.1} min={0} max={1} value={temperature} onChange={e=>setTemperature(Number(e.target.value))}/>
                </label>
                <label className="text-xs" title="Max tokens the model may generate">
                  max <input className="w-20 ml-1 bg-slate-900 border border-slate-700 rounded px-2 py-1" type="number" value={maxTokens} onChange={e=>setMaxTokens(Number(e.target.value))}/>
                </label>
              </>
            )}
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4">
        <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-2 text-xs text-slate-300">
          <label title="Separate datasets; keeps sources isolated">
            collection&nbsp;
            <input className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1" value={collection} onChange={e=>setCollection(e.target.value)} placeholder="default" />
          </label>
          <label title="Chunk size in words/tokens (bigger = fewer, denser chunks)">
            tokens&nbsp;
            <input className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1" type="number" value={tokens} onChange={e=>setTokens(Number(e.target.value))}/>
          </label>
          <label title="Overlap between adjacent chunks (helps context across chunk boundaries)">
            overlap&nbsp;
            <input className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1" type="number" value={overlap} onChange={e=>setOverlap(Number(e.target.value))}/>
          </label>
        </div>
...
function IngestCard(props: {
  ...
  const { urls, setUrls, tokens, setTokens, overlap, setOverlap, force, setForce, runIngest, out, busy, collection } = props;
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
      <div className="font-semibold mb-2">Ingest URLs</div>
      <textarea className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 h-24" value={urls} onChange={e=>setUrls(e.target.value)} />
      <div className="flex flex-wrap gap-2 items-center mt-2 text-sm">
        <span className="bg-slate-800 px-2 py-0.5 rounded" title="Current dataset namespace">collection: {collection}</span>
        <label>tokens <input className="w-20 bg-slate-950 border border-slate-800 rounded px-2 py-1 ml-1" type="number" value={tokens} onChange={e=>setTokens(Number(e.target.value))}/></label>
        <label>overlap <input className="w-20 bg-slate-950 border border-slate-800 rounded px-2 py-1 ml-1" type="number" value={overlap} onChange={e=>setOverlap(Number(e.target.value))}/></label>
        <label className='flex items-center gap-2'><input type="checkbox" checked={force} onChange={e=>setForce(e.target.checked)} /> force</label>
        <button onClick={runIngest} className="bg-emerald-600 hover:bg-emerald-500 rounded-lg px-3 py-1.5" disabled={busy}>{busy?'…':'Ingest'}</button>
